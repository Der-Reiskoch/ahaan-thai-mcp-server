 ...
 * Combines Dictionary, Library (with Book Info), and Encyclopedia
 */
...
server.registerTool(
  "search_dictionary",
  {
    ...
    description:
      "Search for Thai food terms across all categories or within a specific category",
    inputSchema: {
      query: z.string().describe("Search term (Thai, English, or German)"),
      category: z
        .string()
        .optional()
        .describe("Optional category to search within"),
    },
  },
...
server.registerTool(
  ...
  async (filters) => {
    const books = await library.searchBooks(filters);
    return {
      content: [
        {
          type: "text",
...
server.registerTool(
  ...
  async ({ author }) => {
    try {
      const books = await library.getBooksByAuthor(author);
      return {
        content: [
          {
            type: "text",
...
app.get("/", (req, res) => {
  res.json({
    ...
    version: "1.0.1",
    description:
      "MCP server for Thai food data - Dictionary, Library, and Encyclopedia",
    mcp_endpoint: "/mcp",
