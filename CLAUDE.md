...
Three MCP (Model Context Protocol) servers for local usage:

1. **Dictionary Server** (`dictionary-server.js`): Thai food dictionary with translations in Thai, English, and German
2. **Library Server** (`library-server.js`): Thai cookbook metadata and recipes (bilingual: German/English)
3. **Encyclopedia Server** (`encyclopedia-server.js`): Thai food encyclopedia with dishes, ingredients, and cooking methods (bilingual)

### HTTP MCP Server (Remote Usage via HTTP)

Single HTTP MCP server (`src/index.js`) that exposes all MCP functionality via Streamable HTTP transport for remote access. Provides 26 tools combining all 3 data sources.
...
- `src/lib/library-logic.js`: Library business logic (includes book info + recipes)
- `src/lib/encyclopedia-logic.js`: Encyclopedia business logic (bilingual)
...
- **Library API**: `https://www.ahaan-thai.de/api/thai-cook-book-library.json` (combined info + recipes, bilingual)
- **Encyclopedia API (German)**: `https://www.ahaan-thai.de/api/thai-food-encyclopedia.json`
- **Encyclopedia API (English)**: `https://en.ahaan-thai.de/api/thai-food-encyclopedia.json`
...
│       ├── library-logic.js      # Library business logic (book info + recipes, bilingual)
│       └── encyclopedia-logic.js # Encyclopedia business logic (bilingual, fetches 2 APIs)
│
